...
public class CommentSaveService {
    ...
    private final CommentDataRepository commentDateRepository;
    ...
    public void save(RequestComment form) {
        ...
        if (mode.equals("edit") && seq != null) {  // 댓글 수정
            data = commentDateRepository.findById(seq).orElseThrow(CommentNotFoundException::new);

        } else {  // 댓글 추가
            data = new CommentData();
            // 게시글 번호는 변경 x -> 추가할떄 최초 1회만 반영
            ...
            data.setMember(memberUtil.getMember());  // 추가할떄 최초 1회만 반영
        ...
        commentDateRepository.saveAndFlush(data);
